...
| Observed on | `83cda0a`, `11f1661`, `e874c8c`, `148bb31` — four consecutive heads |
...
| — | Whether `reviewedBy` should carry a **reason** and a **hash of the artifact approved**. It carries `operatorUserId`, `displayName` and `reviewedAt` today. `decideReleaseRescueDelivery` binds a decision to the bytes it released, but that is the system's binding, not the reviewer's attestation. Closing it is a schema change and a migration. | this file |
