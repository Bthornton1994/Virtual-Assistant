...
The runtime wrapper claims a lease and attaches the compiled memory receipt, exact selected memory revisions/hashes, and binding hashes in one transaction. The executor sees only the compiled context. The event constraint accepts the binding receipt, and no memory read grants action authority. The runtime schema, lifecycle RPCs, claim RPC, executor authority ceiling, and foreign-key indexes are installed and structurally verified in QA. The rollback-isolated runtime proof covers idempotent plans, duplicate-claim prevention, lease credentials, completion/failure, expiry reaping, cancellation, approval release, and prepare-only executor rejection. True competing-worker concurrency, expiry under load, rollback, and bound-receipt workload tests remain a go-live gate.
