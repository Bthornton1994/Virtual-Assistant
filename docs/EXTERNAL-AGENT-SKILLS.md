...
2. For UI changes, load the one skill that matches the task, following the routing and precedence in `docs/DESIGN_ENGINEERING.md`. Use `frontend-ui-engineering` when building or restructuring components, layouts, or interface states; for a narrower change, use the matching Emil, Jakub, Leon, or ibelick skill instead. Preserve local tokens, primitives, density, and product identity.
