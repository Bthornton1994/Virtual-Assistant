...
describe("work-cell JSON extract", () => {
  ...
  it("finds schemaVersion when pretty-printed with spaces after the brace", () => {
    const parsed = parseExtractedJson('chatter\n{ "schemaVersion" : "catalog-evidence-packet/v1", "runId": "abc" }');
    expect(parsed.ok).toBe(true);
    if (parsed.ok) expect((parsed.value as { runId: string }).runId).toBe("abc");
  });
