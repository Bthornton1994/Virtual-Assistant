...
type TextFile = { path: string; lines: string[] };
...
function findingsFrom(
  ...
  const representable = hits.filter((hit) => {
    const ok = repositoryPathSchema.safeParse(hit.path).success;
  ...
  for (const file of files) {
    ...
    file.lines.forEach((line, index) => {
      for (let offset = 0; offset < Math.max(1, line.length); offset += MAX_SCAN_LENGTH) {
        if (classifyText(line.slice(offset, offset + MAX_SCAN_LENGTH), notes) === "distinctive") {
    ...
    const flush = (endExclusive: number) => {
      ...
      if (classifyText(file.lines.slice(groupStart, endExclusive).join("\n"), null) === "distinctive") {
        hits.push({ path: file.path, startLine: groupStart + 1, endLine: endExclusive });
      }
    };
    file.lines.forEach((line, index) => {
...
export function analyzeSnapshot(snapshot: AcquiredSnapshot): Analysis {
  ...
  for (const file of snapshot.files) {
    if (isBinary(file.bytes)) {
      notes.binaryFilesSkipped += 1;
      continue;
    }
    textFiles.push({ path: file.path, lines: file.bytes.toString("utf8").split(/\r?\n/) });
  }
  ...
  for (const check of RELEASE_RESCUE_RUBRIC_V1) {
    ...
    const status: CheckRunStatus =
      result.findings.length > 0 ? "FAIL" : unreadForTextChecks > 0 ? "BLOCKED" : "PASS";
    checkRuns.push({
      checkId: check.id,
      status,
