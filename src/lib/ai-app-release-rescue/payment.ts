...
export const RESCUE_PAYMENT = {
  ...
  customerCopy: `Checkout is prepared and is not active on this page. When activated, the ${formatUsd(RESCUE_REVIEW_PRICE_USD)} review is charged when an engagement is created. Nothing is billed from this demo.`,
  remediationCopy: `The ${formatUsd(RESCUE_REMEDIATION_PRICE_USD)} remediation sprint is charged only if you accept it after the report. That checkout is also not active here.`,
} as const;
