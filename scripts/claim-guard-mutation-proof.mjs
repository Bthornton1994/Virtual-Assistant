...
const MUTANTS = [
  ...
  {
    id: "M-PASSIVE-AUXILIARY",
    mechanism: "a passive auxiliary belongs to the verb, so a denial reaches through it",
    from: "      if (PASSIVE_AUXILIARIES.has(word)) continue;\n",
    to: "",
    kind: "modelled: the arm removed, which flags the offer's own refusal copy",
    file: "src/lib/release-rescue-intake.ts",
  },
  {
  ...
  {
    id: "M-ESCAPE-BACKSLASH",
    mechanism: "an escaped backslash is consumed whole, so no decode starts inside it",
    from: "const PRINTABLE_ESCAPE = /\\\\\\\\|\\\\u\\{([0-9a-fA-F]+)\\}|\\\\u([0-9a-fA-F]{4})|\\\\x([0-9a-fA-F]{2})/g;",
    to: "const PRINTABLE_ESCAPE = /\\\\u\\{([0-9a-fA-F]+)\\}|\\\\u([0-9a-fA-F]{4})|\\\\x([0-9a-fA-F]{2})/g;",
    kind: "verbatim: the pattern before the escaped-backslash arm",
  },
  {
    id: "M-ESCAPE-LEADING-ZEROS",
    mechanism: "a braced escape's leading zeros are unbounded, as JavaScript allows",
    from: "const PRINTABLE_ESCAPE = /\\\\\\\\|\\\\u\\{([0-9a-fA-F]+)\\}|\\\\u([0-9a-fA-F]{4})|\\\\x([0-9a-fA-F]{2})/g;",
  ...
  {
    id: "M-EDGE-QUOTES-OFFER",
    mechanism: "offer copy is also read with single quotes at a word's edges removed",
    from: "        : [plain, unquoted];",
    to: "        : [plain];",
    kind: "verbatim: offer copy read in its plain tokens alone, as it was",
    file: "src/lib/release-rescue-intake.ts",
  },
  {
    id: "M-EDGE-QUOTES-TYPED",
    mechanism: "a typed value is also read with single quotes at a word's edges removed",
    from: "        ? [plain, withoutPossessives(plain), unquoted, withoutPossessives(unquoted)]",
    to: "        ? [plain, withoutPossessives(plain)]",
    kind: "verbatim: typed values read plain and without possessives, as they were",
    file: "src/lib/release-rescue-intake.ts",
  },
  {
    id: "M-EDGE-QUOTE-BREAK",
    mechanism: "a dropped quote token passes its break to the next token",
    from: "      next = { ...next, breakBefore: strongerBreak(dropped, next), spacedBefore: dropped.spacedBefore || next.spacedBefore };",
    to: "      next = { ...next, spacedBefore: dropped.spacedBefore || next.spacedBefore };",
    kind: "modelled: the quote's break discarded with the quote",
    file: "src/lib/release-rescue-intake.ts",
  },
  {
    id: "M-COLON-EDITION",
    mechanism: "an ISO/IEC 27001 edition written after a colon still reads as the lead-auditor claim",
    from: '  "27001 2005 lead auditor",\n  "27001 2013 lead auditor",\n  "27001 2022 lead auditor",\n',
    to: "",
    kind: "verbatim: the list without the edition phrases, as it was",
    file: "src/lib/release-rescue-intake.ts",
  },
  {
    id: "FP1",
