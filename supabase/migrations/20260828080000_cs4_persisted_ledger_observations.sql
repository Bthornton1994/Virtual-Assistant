...
begin
  ...
  loop
    if jsonb_typeof(v_item) <> 'object'
       or jsonb_typeof(v_item->'observation') <> 'object' then
    ...
    if v_phase not in ('prepare', 'review', 'validate') then
    ...
    if v_observation->>'schemaVersion' <> 'capability-performance-ledger/v1'
       or v_observation->>'runId' <> p_run_id::text
