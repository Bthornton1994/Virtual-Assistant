...
begin
  ...
  if v_spec_id is null then
    insert into public.delegation_specs (
      ...
      '["Grounded repository commit","Grounded static product catalog","product compliance/supply model","primary manufacturer documentation","supplier/partner product source","availability observation with explicit validity window","shipping policy","return policy","recall/compliance sources as applicable"]'::jsonb,
      'prepare_only',
      ...
      '["Deterministic audit output is attached","Repository SHA is recorded","Production-ready candidates pass compliance, supply provenance, fulfillment, seller-of-record, unexpired availability, live inventory, shipping, and returns checks","No unknown or expired value is treated as verified","Full repository verification passes","Independent adversarial review challenges evidence before a passing Outcome Receipt"]'::jsonb,
      '["Escalate conflicting supplier/manufacturer/compliance sources","Preserve unverified/candidate status when evidence is insufficient","Treat expired availability or out-of-stock status as non-production-ready","Stop before vendor contact, contracting, purchasing, publishing, merge, Production commerce, or permission changes"]'::jsonb,
