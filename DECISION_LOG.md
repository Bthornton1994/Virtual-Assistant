...
Narrowed by [D-019](#d-019--wall-clock-scanner-timing-is-an-advisory-observation-counted-work-gates-the-release) (2026-09-23), for the named `timing-observation` check only: its failure is advisory. The wording above is unchanged.
...
## D-019 — Wall-clock scanner timing is an advisory observation; counted work gates the release

Date: 2026-09-23  
Status: **decided — owner-directed**  
Decision: **I adopt Option 3. The deterministic scanner work-count tests and all other existing tests remain release-blocking in `verify`. The wall-clock growth and 2,000 ms timing assertions move to a visible, separately identified, non-required timing-observation check. Failure of that named timing check is advisory and does not block release; its result must remain visible and must not be silently retried. Failures in required `verify` checks remain blockers. This decision expressly narrows D-012’s “Any GitHub Actions failure remains a blocker” clause only for the named timing-observation check. D-018 remains unchanged: `verify` must run the complete SQL proof suite and `proof:claim-guard`. This authorizes recording this decision as D-019 and preparing an isolated draft PR to implement it. It does not authorize merging, deployment, payment activation, production access, permission changes, or increased executor authority.**

Recorded on 2026-09-23 from the owner's instruction, which carried this wording verbatim and directed that it be recorded as D-019. The executor recording it chose none of the words above. "Option 3" is the third option of the owner decision brief on PR [#105](https://github.com/Bthornton1994/Virtual-Assistant/pull/105)'s unresolved timing gate: make the deterministic work-count gate release-blocking, and move wall-clock measurement out of the release-blocking suite.

As implemented: the named check is the `timing-observation` workflow (`.github/workflows/timing-observation.yml`), which runs `npm run test:timing-observation` on pull requests, once, and keeps its observed timings in the job summary and an artifact. The moved assertions are the eleven `stays linear on <shape>` growth-exponent tests and their 2,000 ms ceiling, from `release-rescue-credential-scanner.test.ts`, and the five `stays within budget on <shape>` 2,000 ms tests, from `release-rescue-scanner-value-properties.test.ts`, now in `src/lib/__tests__/release-rescue-scanner.timing-observation.ts` with their thresholds unchanged. `verify` is unchanged and runs lint, typecheck, `npm test`, `proof:claim-guard`, build and `proof:sql`.

What the implementation does NOT do, stated so the record is not read wider than the decision: it changes no threshold; it moves no clock-based assertion other than those two named groups, so the 500 ms intake-parser check, the 1,000 ms detector check and vitest's per-test timeouts remain in `npm test` and remain blocking; it makes no check required or not required in repository settings, which are outside repository content; and it lifts `DO_NOT_MERGE` on no pull request.

Source: owner instruction of 2026-09-23, [D-012](#d-012--the-release-environment-is-github-actions-verify-on-the-exact-candidate-sha), [D-018](#d-018--github-actions-verify-must-execute-the-sql-proofs-and-proofclaim-guard), audit ledger § S-018, PRs [#105](https://github.com/Bthornton1994/Virtual-Assistant/pull/105), [#109](https://github.com/Bthornton1994/Virtual-Assistant/pull/109), [#110](https://github.com/Bthornton1994/Virtual-Assistant/pull/110).
